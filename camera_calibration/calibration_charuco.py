...
images = glob.glob('./hand_eye_calibration/calibration_12192/*.png')
...
CHARUCO_BOARD = cv2.aruco.CharucoBoard((CHARUCOBOARD_COLCOUNT, CHARUCOBOARD_ROWCOUNT), 0.025, 0.019, ARUCO_DICT)
...
for iname in images:
    ...
    if not np.all(ids != None): continue
...
cv_file = cv2.FileStorage("./hand_eye_calibration/charuco_camera_calibration2.yaml", cv2.FILE_STORAGE_WRITE)
